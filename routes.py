...
def configure_routes(app):
    ...
    @token_required
    def get_user_expenses(email, token_claims=None):
        # Ensure token_claims contains the necessary email
        if token_claims and 'email' in token_claims:
            token_email = token_claims['email']
        else:
            token_email = None

        if not email or email != token_email:
            return jsonify({'message': 'Unauthorized'}), 403

        # Now process the request as needed
        user = User.query.filter_by(email=email).first()
        if not user:
            return jsonify({'message': 'User not found'}), 404
        ...
        return jsonify({'message': f'Expenses for {email}'})
    ...
    def get_balance_sheet(email):
        ...
        for email, amount in overall_expenses.items():
            ...
            if user:  # Check if user exists
                ...
                current_app.logger.warning(f"User with email {email} not found in the database.")
...
#     @app.route('/users/<email_param>/expenses', methods=['GET'])
#     @jwt_required()
#     def get_user_expenses(email_param):
#         email = get_jwt_identity()
#         user = User.query.filter_by(email=email_param).first()
#         if not user:
#             return jsonify({'message': 'User not found'}), 404
...
#         email = get_jwt_identity()
#         balance_sheet = get_balance_sheet().get_json()
#         csv_data = "Email,Name,Mobile Number,Total Expense\n"
#         for record in balance_sheet:
#             csv_data += f"{record['email']},{record['name']},{record['mobile_number']},{record['total_expense']}\n"
